...
test("Banner IS Visible when 6 events are returned", async({page})=>
{
    ...
    // it is regex means regular expression
    const banner = page.locator('.text-amber-800');
...
// ── Test 1 By Instructor: 6 events → banner IS visible ──────────────────────────────────────
test.only('sandbox banner is shown when 6 events are returned', async ({ page }) => {
